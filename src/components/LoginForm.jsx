...
export default function LoginForm({setUser}) {
    ...
    function handleChange(evt) {
        ...
        // ...credentials means to extract the data. we then get the name and value
        setError('');
    }
    ...
    return (
        ...
        <form class="max-w-lg border rounded-lg mx-auto" onSubmit={handleSubmit}>
        <div class="flex flex-col gap-4 p-4 md:p-8">
          <div>
            <label for="email" class="inline-block text-gray-800 text-sm sm:text-base mb-2" >Email</label>
            <input name="email" class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" value={credentials.email} onChange={handleChange} required />
          </div>
  
          <div>
            <label for="password" class="inline-block text-gray-800 text-sm sm:text-base mb-2">Password</label>
            <input name="password" class="w-full bg-gray-50 text-gray-800 border focus:ring ring-indigo-300 rounded outline-none transition duration-100 px-3 py-2" value={credentials.password} onChange={handleChange}
            required/>
          </div>
